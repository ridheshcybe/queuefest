...
import fs from 'fs';
fs.writeFileSync(`${process.cwd()}/data/patients.db~`, '', { flag: 'a' });
fs.writeFileSync(`${process.cwd()}/data/queueLogs.db~`, '', { flag: 'a' });
fs.writeFileSync(`${process.cwd()}/data/users.db~`, '', { flag: 'a' });
fs.writeFileSync(`${process.cwd()}/data/patients.db`, '', { flag: 'a' });
fs.writeFileSync(`${process.cwd()}/data/queueLogs.db`, '', { flag: 'a' });
fs.writeFileSync(`${process.cwd()}/data/users.db`, '', { flag: 'a' });
// Create NeDB instances
const patientsInstance = new Datastore({ filename: `${process.cwd()}/data/patients.db`, autoload: true })
const queueLogsInstance = new Datastore({ filename: `${process.cwd()}/data/queueLogs.db`, autoload: true })
const usersInstance = new Datastore({ filename: `${process.cwd()}/data/users.db`, autoload: true })
